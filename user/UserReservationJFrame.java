...
public class UserReservationJFrame extends JFrame implements ActionListener {
	...
	public UserReservationJFrame(String id, String title) throws Exception {
		...
		list = new TCPClient1().userReservation(id);
		...
		if (e.getSource() == reservationButton) {
			if (model.getRowCount() < 3) {
				if (dateChooser.getDate() == null) {
					...
					try {
						ArrayList<ReservationDTO> result = new TCPClient1().getReservation();
						...
						if (count != 0) {
							...
							new TCPClient1().insertReservation(id, title, resDate);
						}
